...
class UpdateGradeRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $gradeId = $this->route('grade') ? $this->route('grade')->id : null;
        
        $rules = [
            'student_id' => 'sometimes|required|exists:users,id',
            'subject_id' => 'sometimes|required|exists:subjects,id',
            'faculty_id' => 'sometimes|required|exists:users,id',
    ...
    public function getValidatedWithCalculatedGrade(): array
    {
        ...
        $semester = $validated['semester'] ?? $this->route('grade')->semester;
        $quarters = [];
        
        if ($semester === '1st') {
            if (isset($validated['first_quarter']) && !is_null($validated['first_quarter'])) {
                $quarters[] = $validated['first_quarter'];
            }
            if (isset($validated['second_quarter']) && !is_null($validated['second_quarter'])) {
                $quarters[] = $validated['second_quarter'];
            }
        } elseif ($semester === '2nd') {
            if (isset($validated['third_quarter']) && !is_null($validated['third_quarter'])) {
                $quarters[] = $validated['third_quarter'];
            }
            if (isset($validated['fourth_quarter']) && !is_null($validated['fourth_quarter'])) {
