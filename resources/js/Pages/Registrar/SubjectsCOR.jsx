...
import { FaPrint, FaArrowLeft, FaBook, FaGraduationCap } from 'react-icons/fa';
...
const SubjectsCOR = () => {
  ...
  const handleStrandChange = (e) => {
    const selectedStrandId = e.target.value;
    if (selectedStrandId) {
      router.get(`/registrar/subjects/cor/${selectedStrandId}`);
    }
  };
  ...
  const renderSubjectsTable = (subjects, yearLevel, semester) => {
    ...
    if (semesterSubjects.length === 0) {
      return (
        <tr>
          <td colSpan="4" className="border border-gray-400 px-3 py-4 text-center text-gray-500 text-sm">
            No subjects for {semester === 1 ? 'First' : 'Second'} Semester
          </td>
        </tr>
      );
    }
    ...
    return semesterSubjects.map((subject, index) => (
      <tr key={subject.id}>
        ...
        <td className="border border-gray-400 px-3 py-2 text-sm text-center">___</td>
      </tr>
    ));
  };
          ...
          <div className="mb-8 print:hidden">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
              <div>
                ...
                <p className="text-gray-600 text-lg">Subject listing for {strand.name} strand</p>
              </div>
              ...
              <div className="text-center">
                ...
                <div className="mt-6 pt-4 border-t-2 border-gray-300 print:border-black">
                  ...
                  <p className="text-lg font-semibold text-gray-800">
                    Academic Track: {strand.name} ({strand.code})
                  </p>
            ...
            <div className="p-8">
              {Object.entries(subjectsByYear).map(([yearLevel, subjects]) => (
                <div key={yearLevel} className="mb-12">
                  <div className={`bg-gradient-to-r ${getStrandColor(strand.code)} p-4 rounded-t-lg print:bg-gray-100 print:border print:border-black`}>
                    <h4 className="text-xl font-bold text-white print:text-black text-center">
                      {yearLevel == 11 ? 'GRADE 11 (First Year)' : 'GRADE 12 (Second Year)'}
                    </h4>
                  </div>
                  
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 bg-gray-50 p-6 print:bg-white print:border print:border-black print:border-t-0">
                    {/* First Semester */}
                    <div>
            ...
            <div className="p-8 border-t-2 border-gray-300 print:border-black bg-gray-50 print:bg-white">
              ...
              <div className="mt-8 text-center">
                <p className="text-xs text-gray-500">
                  This certificate is issued in compliance with DepEd regulations for Senior High School students.
                </p>
